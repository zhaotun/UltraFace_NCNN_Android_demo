...
#define TAG "UltraFaceCpp"
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG,TAG,__VA_ARGS__)

UltraFace::UltraFace(const std::string &model_path,int input_width, int input_length)
{
...
//	iou_threshold = iou_threshold_;
	...
	std::vector<std::string> param_files = {
			model_path+"/RFB-320.param",
	};
	std::vector<std::string> bin_files = {
			model_path+"/RFB-320.bin",
	};

	//init
	ultraface.load_param(param_files[0].data());
	ultraface.load_model(bin_files[0].data());
...
void UltraFace::SetNumThreads(int numThreads){
